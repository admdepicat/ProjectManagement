...
| 実装 | 場所 | データ保存先 | 向いている人 |
|---|---|---|---|
| **Next.js 版** | リポジトリ直下 | SQLite (`data/app.db`) | 自分でサーバーを動かせる/ローカル利用 |
| **GAS 版** | [`gas/`](./gas/) | **Google スプレッドシート + Drive** | サーバー不要。スプレッドシートにデータを残したい・Google Workspace で完結させたい |

GAS 版はスプレッドシートをDBとして使い、音声ファイルは Google Drive に保存、
Apps Script の Web アプリとしてそのまま動きます。セットアップ手順は [gas/README.md](./gas/README.md) を参照。
