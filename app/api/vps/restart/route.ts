...
export async function POST(request: NextRequest) {
  ...
  return new Promise<NextResponse>((resolve) => {
    ...
    const timeout = setTimeout(() => {
      conn.end();
      resolve(
        NextResponse.json({ error: "Connection timed out" }, { status: 504 })
      );
    }, 15000);
    ...
    conn
        ...
        conn.exec("sudo reboot", (err, stream) => {
          ...
          stream
            .on("close", () => {
              ...
              conn.end();
              resolve(
                NextResponse.json({ success: true, message: "VPS reboot command sent" })
              );
            })
            .on("data", () => {})
            .stderr.on("data", () => {});
      ...
      .on("error", (err) => {
        clearTimeout(timeout);
        resolve(
          NextResponse.json({ error: `SSH connection failed: ${err.message}` }, { status: 502 })
        );
      })
